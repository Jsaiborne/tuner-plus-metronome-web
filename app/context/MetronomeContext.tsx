...
type MetronomeContextType = {
  ...
  current16thNote: number; // legacy name, now represents the current tick index according to subdivision
  setCurrent16thNote: React.Dispatch<React.SetStateAction<number>>;
  tempo: number;
  setTempo: React.Dispatch<React.SetStateAction<number>>;
  lookahead: number;
  setLookAhead: React.Dispatch<React.SetStateAction<number>>;
  scheduleAheadTime: number;
  setScheduleAheadTime: React.Dispatch<React.SetStateAction<number>>;
  nextNoteTime: number;
  setNextNoteTime: React.Dispatch<React.SetStateAction<number>>;
  noteResolution: number; // 0 = 16th, 1 = 8th, 2 = quarter, 3 = triplet
  setNoteResolution: React.Dispatch<React.SetStateAction<number>>;
  noteLength: number;
  ...
  beatUnit: number;
  setBeatUnit: React.Dispatch<React.SetStateAction<number>>;
};

export const MetronomeContext = createContext<MetronomeContextType | undefined>(undefined);
...
export const MetronomeProvider: React.FC = () => {
  ...
  const [lookahead, setLookAhead] = useState(25.0); // internal only
  ...
  useEffect(() => { if (workerRef.current) { try { workerRef.current.postMessage({ interval: lookaheadRef.current }); } catch {} } }, [lookaheadRef.current]);
  ...
  const contextValue: MetronomeContextType = {
    ...
    isPlaying,
    setIsPlaying,
    current16thNote: current16thNoteState,
    setCurrent16thNote: setCurrent16thNoteState,
    tempo,
    setTempo,
    lookahead,
    setLookAhead,
    ...
    nextNoteTime: nextNoteTimeState,
    setNextNoteTime: setNextNoteTimeState,
    noteResolution,
    setNoteResolution,
    noteLength,
    setNoteLength,
